...
class DB():
    ...
    @classmethod
    def query(cls, cql, page=None, limit=None):
        if cls.q:
            status, rows, tm, page =  cls.q.query(cql, page, limit=limit)
            if status != 'ok':
                flash(str(status))
            return rows, tm, page
        else:
            return None, 0, None # status, rows, query_time, page_state
...
def global_data():
    ...
    try:
        rows, tm, page = DB.query('SELECT keyspace_name, table_name FROM system_schema.tables', limit=0)
    except Exception as e:
        flash('Error: %s' %str(e))
        return dict()
    ...
    for row in rows or []:
        if not row['keyspace_name'].startswith('system'):
            if row['keyspace_name'] not in keyspaces:
                keyspaces[row['keyspace_name']] = []
            keyspaces[row['keyspace_name']].append(row['table_name'])
    return dict(query_time=tm, keyspaces=keyspaces, cql=cql)
...
def db():
    ...
    if request.method == 'POST':
        ...
        if ssh and not (ssh_user and ssh_password):
        ...
        if ip and port and user and password:
            ...
            if ssh:
                ret = DB.ssh(ip, ssh_user, ssh_password)
                if ret:
                    flash('Error: %s' %ret)
                    return redirect(url_for('main'))
